...
- Container images (`Dockerfile`, `Dockerfile.bundle`) are pinned by digest, not just tag, for reproducible builds; Dependabot (`.github/dependabot.yml`) opens a PR when a pinned digest moves. A CVE published against a package in the pinned base fails the `container-scan` job on every branch until the digest is bumped — see [Refreshing a pinned base image after a CVE](SECURITY.md#refreshing-a-pinned-base-image-after-a-cve).
